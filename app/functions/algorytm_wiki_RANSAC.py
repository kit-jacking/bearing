...
def visualize_points(left_point, right_point, center_point, depth_point, points_on_plane, line_points,
                     depth_line_points, point_cloud: o3d.geometry.PointCloud):
...
def find_fit(arc_filepath, points_filepath):
    if arc_filepath.endswith(".ply") or arc_filepath.endswith(".pcd"):
        arc_o3d: o3d.geometry.PointCloud = o3d.io.read_point_cloud(arc_filepath)
    arc_pts = np.asarray(arc_o3d.points)

    if points_filepath.endswith(".ply") or points_filepath.endswith(".pcd"):
    ...
    plane_equation = equation_plane(center_point[0], center_point[1], center_point[2], left_point[0], left_point[1],
                                    left_point[2], right_point[0], right_point[1], right_point[2])
    ...
    # Dopasowanie punktow 
    regressor = RANSAC(model=HyperbolicRegressor(), loss=square_error_loss, metric=mean_square_error)
    regressor.fit(X, y)
    ...
    visualize_points(left_point, right_point, center_point, depth_point, points_on_plane, line_points,
                     depth_line_points, arc_o3d)
